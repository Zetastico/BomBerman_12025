...
void ABomBerman_012025Character::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		//Putting Bomb
		EnhancedInputComponent->BindAction(BombAction, ETriggerEvent::Started, this, &ABomBerman_012025Character::Bomba);
	}
	else
	{
...
void ABomBerman_012025Character::Bomba(const FInputActionValue& Value)
{

	FVector Posicion = GetActorLocation() + GetActorForwardVector() * 100.f;
	Posicion = AlineadoAGrilla(Posicion);
	
	GetWorld()->SpawnActor<ABombaBase>(ABombaBase::StaticClass(), Posicion, FRotator::ZeroRotator);

}

FVector ABomBerman_012025Character::AlineadoAGrilla(FVector Posicion)
{
	const float TamanoCelda = 100.0f;
	return FVector(
		FMath::RoundToFloat(Posicion.X / TamanoCelda) * TamanoCelda,
		FMath::RoundToFloat(Posicion.Y / TamanoCelda) * TamanoCelda,
		100.0f); // Suponemos Z=0 para grilla plana
}