...
void AEnemigo_Observer::BeginPlay()
{
    Super::BeginPlay();

    bJugadorDetectadoBomb = false;
...
void AEnemigo_Observer::Tick(float DeltaTime)
{
    ...
    {
        bEsperando = true;
        GetWorldTimerManager().SetTimer(TimerEspera, this, &AEnemigo_Observer::IrAlSiguientePunto, TiempoEspera, false);
    }
    else if (!bJugadorDetectadoBomb)
    {
        ...
        {
            bEsperando = true;
            GetWorldTimerManager().SetTimer(TimerEspera, this, &AEnemigo_Observer::ReanudarPatrulla, TiempoEspera, false);
...
void AEnemigo_Observer::IrAlSiguientePunto()
{
...
void AEnemigo_Observer::ActivarExplosion()
{
    GetWorld()->GetTimerManager().SetTimer(TimerHandle_Explosion, this, &AEnemigo_Observer::Explotar, TiempoParaExplotar, false);
}

void AEnemigo_Observer::Explotar()
{
    if (EfectoExplosion)
    {
...
void AEnemigo_Observer::ExplorarEnDireccion(FVector Direccion)
{
...
void AEnemigo_Observer::ReanudarPatrulla()
{
    {
...
void AEnemigo_Observer::Desuscribir()
{
	if (!BloqueTrampa) {
		return;
	}
			//Unsubscribe from the Clock Tower if it's destroyed
	BloqueTrampa->DeSubscribir(this);
}

void AEnemigo_Observer::Update(ANotificador* Publisher)
{
    bJugadorDetectadoBomb = true;
}

void AEnemigo_Observer::SetBloqueTrampa(ABloque_Trampa* miBloqueTrampa)
{
   if (!miBloqueTrampa) {
            return;
   }
   BloqueTrampa = miBloqueTrampa;
   BloqueTrampa->Subscribir(this);
